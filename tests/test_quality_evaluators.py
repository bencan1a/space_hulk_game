...
class TestPlotEvaluator(unittest.TestCase):
    ...
    def test_evaluate_good_plot(self):
        ...
        # Create comprehensive plot JSON with sufficient word count
        plot_json = """
{
  ...
  "themes": ["horror", "survival", "duty"],
  "plot": {
    "prologue": "A team of elite Space Marines from the Ultramarines chapter receives a distress signal from a derelict space hulk. The hulk, identified as the ancient vessel 'Emperor's Grace', has been missing for over three centuries after disappearing into a warp storm during the Horus Heresy. The Space Marines are ordered to board the hulk immediately to investigate the mysterious distress signal and retrieve any valuable technology or lost relics of the Imperium.",
    "act1": "The Space Marines begin their methodical exploration of the docking bay, discovering the frozen corpses of the original crew still at their stations. Ancient technology hums with an eerie power, and the air is thick with the stench of corruption. As they venture deeper into the hulk, they must navigate through sections of zero-gravity where the artificial gravity has failed, avoid deadly traps set by long-dead defenders, and deal with malfunctioning automated defense systems. Branching Path 1: A) Split the team - one squad investigates engineering, another secures the bridge B) Keep the team together for maximum firepower and safety",
    "act2": "As the Marines penetrate deeper into the hulk's heart, they encounter the first wave of Genestealers, a horrifying discovery that the hulk is infested with a full brood of these deadly xenos creatures. The team must fight for their lives in the narrow, claustrophobic corridors. During a brief respite after a fierce battle, they discover that they are not alone - a Dark Angels strike force arrived at the hulk days earlier. Branching Path 2: A) Trust the Dark Angels and combine forces against the Genestealer threat B) Pursue objectives independently, suspicious of Dark Angels' ulterior motives"
  },
  "endings": [
    {
      "name": "Victory",
...
"""
        ...
        result = self.evaluator.evaluate(plot_json)
...
"""
    ...
    def test_evaluate_invalid_json(self):
        ...
        invalid_json = '{ "invalid": "json", "unclosed": ['

        result = self.evaluator.evaluate(invalid_json)

        self.assertEqual(result.score, 0.0)
        self.assertFalse(result.passed)
        self.assertIn("error", result.details)

    def test_evaluate_json_with_special_characters(self):
        """Test evaluating JSON with special characters (colons, quotes, etc.)."""
        plot_json = """
{
  "title": "Space Hulk: Derelict of the Damned",
  "setting": {
  ...
  "plot": {
    ...
    "act1": ["Branching Path 1: A) Split team B) Stay together"],
    "act2": ["Branching Path 2: A) Trust allies B) Go alone"]
  },
  "endings": [
    {"name": "Victory", "type": "victory"},
    {"name": "Defeat", "type": "defeat"}
  ]
}
"""

        # Should successfully parse JSON with special characters
        result = self.evaluator.evaluate(plot_json)
...
"""
        ...
        feedback = self.evaluator.generate_detailed_feedback(plot_json)
...
"""
        ...
        result = self.evaluator.evaluate(narrative_json)
...
"""
        ...
        result = self.evaluator.evaluate(puzzle_json)
        ...
        scene_json = """
{
  "scenes": {
    "scene_1": {
      "description": "The docking bay stretches before you, a vast cathedral of rust and shadows. Ancient machinery looms overhead, covered in frost and grime. The air tastes of metal and decay. Frozen corpses stand at their stations like grotesque statues, their faces locked in eternal screams. Warning lights flicker red across the walls, casting everything in a hellish glow."
    },
    "scene_2": {
      "description": "You enter the engineering deck, where massive generators pulse with eerie green light. The sound of humming machinery fills your ears. Strange symbols glow on the control panels, pulsing in rhythm with your heartbeat. The floor vibrates beneath your feet. A cold wind blows from the ventilation shafts."
    },
    "scene_3": {
      "description": "The command bridge is a monument to lost glory. Throne-like command chairs face cracked viewscreens showing only static. Dust covers every surface. Your footsteps echo in the silence. Ancient cogitators click and whir, processing data from centuries past."
    },
    "scene_4": {
      "description": "Crew quarters line both sides of the corridor. Personal effects scatter the floor - photos, medals, prayer beads. The smell of decay permeates everything. Bunks are still made, waiting for crew that will never return."
    },
    "scene_5": {
      "description": "The generator room thrums with power. Massive turbines spin endlessly, their purpose long forgotten. Coolant pipes hiss and drip. Control panels display warnings in dead languages. The heat is oppressive, making you sweat."
    },
    "scene_6": {
      "description": "A maintenance tunnel winds through the ship's skeleton. Exposed wiring crackles with electricity. The walls press in close. You must crouch to move forward. Distant sounds echo - metallic screeches, inhuman howls."
    }
  }
}
"""

        result = self.evaluator.evaluate(scene_json)
...
{
  "health_system": {
    "description": "Players have health points representing physical condition. Health ranges from 0 (dead) to 100 (full health). Taking damage reduces health. Healing items and rest restore health. At 0 health, the game ends in defeat."
  },
  "inventory_system": {
    "description": "Players can carry up to 10 items. Items include weapons, tools, keycards, and consumables. Items can be picked up from scenes and used to solve puzzles or overcome obstacles. Some items are required to progress."
  },
  "combat_system": {
    "description": "Combat is turn-based. Players choose actions: attack, defend, use item, or flee. Enemies have health and attack values. Success depends on weapon type and player choices. Defeating enemies may yield items or access to new areas."
  },
  "movement_system": {
    "description": "Players navigate between connected scenes. Each scene has a description, available items, NPCs, and exits. Some areas are locked until puzzles are solved or items are found. The map reveals as players explore."
  }
}
"""

        result = self.evaluator.evaluate(mechanics_json)
...
class TestEvaluatorIntegration(unittest.TestCase):
    ...
    def test_all_evaluators_return_quality_score(self):
        ...
        plot_json = '{"title": "Test", "setting": "Place", "themes": ["horror"], "plot": {"act1": "story"}, "endings": [{"name": "end", "type": "victory"}]}'
        narrative_json = '{"scenes": {"s1": {"description": "desc", "connections": []}}}'
        puzzle_json = '{"puzzles": [{"id": "p1", "solution": "yes", "narrative_tie": "yes", "difficulty": "easy"}]}'
        scene_json = '{"scenes": {"s1": {"description": "A vivid description"}}}'
        mechanics_json = '{"systems": {"combat": {"description": "How combat works"}}}'
        ...
        contents = [plot_json, narrative_json, puzzle_json, scene_json, mechanics_json]
    ...
    def test_evaluators_handle_markdown_wrapped_json(self):
        """Test that evaluators handle markdown-wrapped JSON."""
        plot_json = """```json
{
  "title": "Test Plot",
  "setting": "A place",
...
```"""
        ...
        result = evaluator.evaluate(plot_json)

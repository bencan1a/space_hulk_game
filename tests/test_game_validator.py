...
class TestGameValidator(unittest.TestCase):
    ...
    def test_validate_simple_valid_game(self):
        ...
        scene = Scene(
            id="start", name="Starting Room", description="A simple starting room.", exits={}
        )
    ...
    def test_validate_connected_game(self):
        ...
        scene3 = Scene(
            id="vault", name="Vault", description="The treasure vault.", exits={"west": "corridor"}
        )

        game_data = GameData(
            title="Connected Game",
            description="A game with connected scenes",
            scenes={"entrance": scene1, "corridor": scene2, "vault": scene3},
    ...
    def test_detect_unreachable_scenes(self):
        ...
        scene1 = Scene(
            id="start", name="Start", description="Starting point.", exits={"north": "room1"}
        )
        scene2 = Scene(
            id="room1", name="Room 1", description="First room.", exits={"south": "start"}
        )
        ...
        game_data = GameData(
            ...
            scenes={"start": scene1, "room1": scene2, "orphan": scene3},
            starting_scene="start",
        )
    ...
    def test_detect_invalid_exits(self):
        ...
        scene = Scene(
            id="start", name="Start", description="Starting point.", exits={"north": "missing_room"}
        )
        ...
        self.assertTrue(
            any(
                "invalid exit" in issue.lower() and "missing_room" in issue
    ...
    def test_detect_dead_ends(self):
        ...
        scene1 = Scene(
            id="start", name="Start", description="Starting point.", exits={"north": "dead_end"}
        )
        scene2 = Scene(
            id="dead_end",
            name="Dead End",
        ...
        game_data = GameData(
            ...
            scenes={"start": scene1, "dead_end": scene2},
            starting_scene="start",
        ...
        self.assertTrue(
            any("dead_end" in warning.lower() for warning in result.warnings)
            or any("dead_end" in issue.lower() for issue in result.issues)
        )

        # Should suggest adding exits
        if "dead_end" in result.suggestions:
            self.assertTrue(any("exit" in s.lower() for s in result.suggestions["dead_end"]))

    def test_dead_end_ok_if_ending(self):
        """Test that dead ends are OK if marked as endings."""
        scene1 = Scene(
            id="start", name="Start", description="Starting point.", exits={"north": "ending"}
        )
        scene2 = Scene(
            id="ending",
            name="The End",
        ...
        game_data = GameData(
            ...
            scenes={"start": scene1, "ending": scene2},
            starting_scene="start",
    ...
    def test_detect_missing_npc_item(self):
        ...
        scene = Scene(id="start", name="Start", description="Starting point.", npcs=[npc])
        ...
        self.assertFalse(result.is_valid())
        self.assertTrue(any("magic_sword" in issue for issue in result.issues))

    def test_npc_item_exists_globally(self):
        """Test that NPC items are OK if they exist in global items."""
        magic_sword = Item(id="magic_sword", name="Magic Sword", description="A legendary blade.")

        npc = NPC(id="merchant", name="Merchant", description="A trader.", gives_item="magic_sword")
    ...
    def test_detect_npc_no_dialogue(self):
        ...
        self.assertTrue(any("dialogue" in warning.lower() for warning in result.warnings))
    ...
    def test_complex_game_validation(self):
        ...
        scene4 = Scene(id="orphan", name="Orphan", description="Unreachable.", exits={})
        ...
        game_data = GameData(
            ...
            scenes={"start": scene1, "room1": scene2, "locked_room": scene3, "orphan": scene4},
            starting_scene="start",
        ...
        self.assertTrue(any("orphan" in issue.lower() for issue in result.issues))

        # Check for invalid exit
        self.assertTrue(any("missing" in issue.lower() for issue in result.issues))

        # Check for missing NPC item
        self.assertTrue(any("missing_item" in issue for issue in result.issues))
    ...
    def test_find_reachable_scenes(self):
        ...
        scene1 = Scene(id="a", name="A", description="A", exits={"to_b": "b"})
        scene2 = Scene(id="b", name="B", description="B", exits={"to_c": "c", "back": "a"})
        scene3 = Scene(id="c", name="C", description="C", exits={"back": "b"})
        scene4 = Scene(id="d", name="D", description="D (unreachable)", exits={})

        scenes = {"a": scene1, "b": scene2, "c": scene3, "d": scene4}
    ...
    def test_locked_exit_validation(self):
        ...
        game_data = GameData(
            ...
            scenes={"start": scene1, "locked_room": scene2},
            starting_scene="start",
        )
    ...
    def test_stats_collection(self):
        ...
        scene = Scene(id="start", name="Start", description="Start", items=[item], npcs=[npc])
    ...
    def test_strict_mode(self):
        ...
        npc = NPC(id="silent_npc", name="Silent NPC", description="A quiet character")
        scene = Scene(id="start", name="Start", description="Starting room", exits={}, npcs=[npc])

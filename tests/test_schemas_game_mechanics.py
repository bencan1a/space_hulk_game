...
class TestInventorySystem:
    ...
    def test_inventory_capacity_constraints(self):
        ...
        with pytest.raises(ValidationError):
            InventorySystem(
                description="A" * 50, capacity=0,  # Too low
                commands=["take"], narrative_purpose="B" * 50
            )
        with pytest.raises(ValidationError):
            InventorySystem(
                description="A" * 50, capacity=101,  # Too high
                commands=["take"], narrative_purpose="B" * 50
            )
    ...
    def test_valid_combat_system(self):
        ...
        combat = CombatSystem(
            ...
            mechanics=[
                CombatMechanic(
                    name="Overwatch",
                    rules="Set a character to fire upon enemies..."
                )
            ],
            narrative_purpose="B" * 50
        )
        ...
        state = GameState(
            tracked_variables=[
                TrackedVariable(
                    ...
                    purpose="Tracks mental state"
                )
            ],
            win_conditions=[
                WinCondition(
                    condition="Successfully complete the mission"
                )
            ],
            lose_conditions=[
                LoseCondition(
                    condition="All squad members killed"
                )
            ]
        )
        ...
        mechanics = GameMechanics(
            ...
            game_systems=GameSystems(
                ...
                combat=CombatSystem(
                    description="A" * 50,
                    mechanics=[
                        CombatMechanic(name="Test", rules="Test rules here")
                    ],
                    narrative_purpose="B" * 50
                ),
            ...
            game_state=GameState(
                tracked_variables=[
                    TrackedVariable(variable="var1", purpose="Purpose here")
                ],
                win_conditions=[
                    WinCondition(condition="Win condition here")
                ],
                lose_conditions=[
                    LoseCondition(condition="Lose condition here")
                ]
            ),

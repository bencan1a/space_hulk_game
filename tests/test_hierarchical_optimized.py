...
def test_optimized_hierarchical():
    ...
    print(f"Test Mode: Hierarchical with optimizations")
    print(f"Optimizations:")
    print(f"  - Reduced max_iter: 10 (down from 25)")
    print(f"  - Simplified manager with lower temperature (0.3)")
    print(f"  - Limited token usage (4000 max)")
    print("="*80 + "\n")
    ...
    try:
        ...
        print("="*80)
        print(f"✅ Hierarchical mode SUCCEEDED with optimizations")
        ...
        print(f"  - Manager max_iter: 10 (prevents excessive delegation)")
        print(f"  - Manager temperature: 0.3 (more consistent decisions)")
        print(f"  - Manager max_tokens: 4000 (prevents context overflow)")
        ...
        logger.error(f"Optimized hierarchical mode failed: {str(e)}", exc_info=True)
        return False

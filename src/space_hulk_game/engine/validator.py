...
class GameValidator:
    ...
    def validate_game(self, game_data: GameData) -> ValidationResult:
        ...
        logger.info(
            f"Validation complete: {len(result.issues)} issues, "
    ...
    def _check_scene_reachability(
        ...
        reachable = self._find_reachable_scenes(
            game_data.scenes,
            game_data.starting_scene
        )
        ...
        if unreachable:
            ...
            result.add_issue(
                f"Unreachable scenes: {', '.join(unreachable_list)}"
            )
            ...
            for scene_id in unreachable_list:
                ...
                if reachable:
                    ...
                    result.add_suggestion(
                        closest,
                        f"Add exit to unreachable scene '{scene_id}'"
                    )
    
    def _check_invalid_exits(
        self,
        game_data: GameData,
        result: ValidationResult
    ) -> None:
        ...
        for scene_id, scene in game_data.scenes.items():
            for direction, target_id in scene.exits.items():
                if target_id not in game_data.scenes:
                    ...
                    result.add_suggestion(
                        scene_id,
                        f"Remove invalid exit '{direction}' or create scene '{target_id}'"
                    )
    ...
    def _check_dead_ends(
        ...
        if dead_ends:
            ...
            for scene_id in dead_ends:
                result.add_suggestion(
                    scene_id,
                    f"Add exits to other scenes or mark as an ending scene"
                )
    ...
    def _check_npc_dialogues(
        ...
        for scene_id, scene in game_data.scenes.items():
            for npc in scene.npcs:
                if not npc.dialogue:
                    result.add_warning(
                        f"NPC '{npc.id}' in scene '{scene_id}' has no dialogue"
                    )
                    result.add_suggestion(
                        scene_id,
                        f"Add dialogue for NPC '{npc.id}'"
                    )
